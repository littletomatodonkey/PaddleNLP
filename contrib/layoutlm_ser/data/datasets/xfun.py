...
class XFUN(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        XFUNConfig(
            name=f"xfun.{lang}", lang=lang) for lang in _LANG
    ]
        ...
        return datasets.DatasetInfo(
            features=datasets.Features({
                ...
                "entities": datasets.Sequence({
                    ...
                    "label":
                    datasets.ClassLabel(names=["HEADER", "QUESTION", "ANSWER"]),
                }),
    ...
    def _split_generators(self, dl_manager):
        ...
        urls_to_download = {
            ...
            "val": [
                f"{_URL}{self.config.lang}.val.json",
                f"{_URL}{self.config.lang}.val.zip"
            ],
        ...
        if self.config.additional_langs:
            ...
            if "all" in additional_langs:
                additional_langs = [
                    lang for lang in _LANG if lang != self.config.lang
                ]
            for lang in additional_langs:
                urls_to_download = {
                    "train":
                    [f"{_URL}{lang}.train.json", f"{_URL}{lang}.train.zip"]
                }
                additional_downloaded_files = dl_manager.download_and_extract(
                    urls_to_download)
                train_files_for_many_langs.append(additional_downloaded_files[
                    "train"])
    ...
    def _generate_examples(self, filepaths):
        for filepath in filepaths:
            ...
            for doc in data["documents"]:
                doc["img"]["fpath"] = os.path.join(filepath[1],
                                                   doc["img"]["fname"])
                ...
                for line in document:
                    ...
                    relations.extend(
                        [tuple(sorted(l)) for l in line["linking"]])
                    ...
                    for token_id, offset in zip(
                        ...
                        while ocr_length < text_length:
                            ...
                            ocr_length += len(
                                self.tokenizer._tokenizer.normalizer.
                                normalize_str(ocr_word["text"].strip()))
                        ...
                        last_box = tmp_box
                    bbox = [[
                        bbox[i + 1][0], bbox[i + 1][1], bbox[i + 1][0],
                        bbox[i + 1][1]
                    ] if b is None else b for i, b in enumerate(bbox)]
                    if line["label"] == "other":
                        label = ["O"] * len(bbox)
                    else:
                    ...
                    for i in tokenized_doc:
                        tokenized_doc[i] = tokenized_doc[i] + tokenized_inputs[
                            i]
                ...
                for chunk_id, index in enumerate(
                    ...
                    for entity_id, entity in enumerate(entities):
                        if (index <= entity["start"] < index + chunk_size and
                            ...
                            global_to_local_map[entity_id] = len(
                                entities_in_this_span)
                            entities_in_this_span.append(entity)
                    relations_in_this_span = []
                    for relation in relations:
                        if (index <= relation["start_index"] <
                                index + chunk_size and index <=
                                relation["end_index"] < index + chunk_size):
